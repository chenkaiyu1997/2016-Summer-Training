...
bool inangle(const point& p1,const point& pl,const point& pr,const point& toc) {
	...
	bool flag=true;
	if(dcmp(cross(vecr,vecl))<0) flag=false;
	if(flag){
		if(dcmp(cross(vecr,vecc))>=0 && dcmp(cross(vecc,vecl))>=0)return true;
		return false;
	}
	else{
		if(dcmp(cross(vecl,vecc))>0 && dcmp(cross(vecc,vecr))>0)return false;
		return true;
	}
}
...
void checkrec(const point& p1,point pl,point pr) {
	point v1=pl-p1,v2=pr-p1;
	if(dcmp(cross(v2,v1))<0)swap(pl,pr);
	point p4=pr+pl-p1;
	for(int i=1;i<=n;i++)
...
void checkwhilemoving(double t) {
	...
	checkrec(s,tmpvr,mvt);
	checkrec(s,tmpvl,mvt);
